...
class I3Res50(nn.Module):
    ...
    def forward_single(self, x):
        ...
        x = self.maxpool1(x)
        # print('maxpool1  x:', x.shape)

        x = self.layer1(x)
        # print('layer1    x:', x.shape)
        x = self.maxpool2(x)
        # print('maxpool2  x:', x.shape)
        x = self.layer2(x)
        # # print('layer2    x:', x.shape)
        x = self.layer3(x)
        # # print('layer3    x:', x.shape)
        roi_feat = self.layer4(x)
        # print('layer4    x:', roi_feat.shape)              # 1, 2048, T, 7, 7

        avg_feat = self.avgpool(roi_feat.permute(2,1,0,3,4)) # T, 2048, 1, 1, 1

        feat = avg_feat.view(avg_feat.shape[0], 1, -1)       # T, 1, 2048
        # print('resnet feat.shape:', feat.shape)
        # feat = torch.nn.MaxPool1d(4)(feat)                  # T, 1, 128
        # print('feat.shape:', feat.shape)

        # print('avgpool   x:', avg_feat.shape)
        # avg_feat = self.drop(avg_feat)
        # print('drop      x:', avg_feat.shape)
        #
        # avg_feat = avg_feat.view(avg_feat.shape[0], -1)
        # print('view      x:', avg_feat.shape)
        # avg_feat = self.fc(avg_feat)
        # print('   avg_feat:', avg_feat.shape)

        # print('-' * 80)


        return feat # roi_feat, avg_feat, feat
    ...
    def forward(self, batch):
        ...
        if inputs.dim() == 5:
            pred = self.forward_single(inputs)#batch['frames'])

        # 7D tensor == 3 crops/10 clips
        elif inputs.dim() == 7:
            pred = self.forward_multi(inputs)#['frames']
