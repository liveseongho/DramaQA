...
class ABC(BaseModel):
    def __init__(self, pt_emb, **kwargs):
        super().__init__()
        self.vid_flag = True#"imagenet" in opt.input_streams
        self.sub_flag = True#"sub" in opt.input_streams
        self.vcpt_flag = True#"vcpt" in opt.input_streams
        hidden_size_1 = 150
        hidden_size_2 = 300
        n_layers_cls = 1
        vid_feat_size = 512
        embedding_size = 300
        vocab_size = pt_emb.shape[0]
    ...
    def forward(self, x):
        q, q_l = x['que'], x['que_l']
        a0, a1, a2, a3, a4, = x['ans'].transpose(0,1)
        a0_l, a1_l, a2_l, a3_l, a4_l = x['ans_l'].transpose(0,1)
        sub, sub_l = x['filtered_sub'], x['filtered_sub_len']
        vcpt, vcpt_l = x['vmeta'], x['bbfts_l']
        vid, vid_l = x['bbfts'], x['bbfts_l']
        ...
        return out.squeeze().view(e_q.shape[0], -1)
